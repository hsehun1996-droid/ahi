...
- **모식도에서 선택**: 사업대상/운영계획변경 창의 ‘모식도에서 선택’ 버튼 → 메인 모식도가 DI 지수(포장상태불량률) **+ 보수이력(공법 색상)** 동시 표시로 전환 → 상행/하행 바의 **차로별로 드래그**하여 km 구간 선택(100m 스냅, Shift=추가, 선택 클릭=해제, ESC=전체 해제). 보수이력 구간·교량/터널 구간은 빗금(선택불가)으로 표시되며 드래그 시 자동으로 잘라냄. ‘선택 적용’ 시 우선순위 산정과 동일 경로로 표에 추가됨
...
> 참고: `select_mixin`의 모식도 선택 모드는 `on_improvement_priority`와 **동일한 `apply_callback`**(`_bp_apply_priority`/`_oc_apply_priority`)으로 결과를 넘긴다. 진입 시 메인 모식도를 DI 보기로 전환하되 보수이력(공법 색상)도 함께 그려(`draw_schematic`에서 `schematic_select_mode`이면 연도필터 무시·`_draw_entries` 호출) DI와 이력을 동시에 보여준다. 보수이력·구조물 구간은 `_selection_blocked_intervals`로 산정해 **빗금만**(어둡게 가리지 않음, 구조물처럼 내용은 보이되 선택불가) 표시하고, 드래그 구간에서 이 구간들을 `_subtract_intervals`로 잘라낸다. 선택불가 판정은 **차로별**(`lane_conflict`)이며, 오버레이는 `draw_schematic` 말미에서만 그려진다(IC 갭은 `route["_gaps_px"]`로 분절). ESC는 모드 종료가 아니라 **선택 전체 해제**로 동작한다(`exit_magnifier_mode`가 선택 모드를 우선 처리).
